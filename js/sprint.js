...
function renderSprint() {
  ...
  html += '<div class="qa-answer-text">' + esc(intro.sixty_seconds) + '</div>';
  html += '<div style="display:flex;gap:.5rem;flex-wrap:wrap;padding:.75rem 1.2rem">';
  html += '<button class="btn btn-sm" onclick="playScript(\'intro-60s.mp3\', this)">Listen (Loop)</button>';
  ...
  html += '<div style="display:flex;gap:.5rem;flex-wrap:wrap;padding:.75rem 1.2rem">';
  html += '<button class="btn btn-sm" onclick="playScript(\'sprint-q3-full.mp3\', this)">Listen EN</button>';
  html += '</div>';
  ...
  /* ========== Defensive Quick Answers ========== */
  html += '<h2 style="color:#ef4444;border-bottom:1px solid #2d3148;padding-bottom:.5rem;margin-top:2.5rem">防禦速答 Defensive Answers</h2>';
  html += '<p style="color:#9ca3af;font-size:.85rem;margin-bottom:1rem">被挑戰時的快速回應 — 不要防守，要轉攻</p>';

  // Defense 1
  html += '<div class="tip-card" style="border-left-color:#ef4444">';
  html += '<div class="tip-title">「你沒有半導體經驗」</div>';
  html += '<div class="tip-content" style="white-space:pre-wrap">My background is in software and SaaS rather than semiconductor. But change management is fundamentally about people and processes — understanding resistance, building trust, proving value through results. Those challenges are universal.\n\nAnd I\'ve consistently shown I can ramp up fast — at CINNOX, I went from zero telecom knowledge to rebuilding the core VoIP product in three months.</div>';
  html += '<div style="color:#6b7280;font-size:.8rem;margin-top:.5rem">方法論跨產業，工程背景學很快</div>';
  html += '</div>';

  // Defense 2
  html += '<div class="tip-card" style="border-left-color:#ef4444">';
  html += '<div class="tip-title">「你的 AI 經驗規模太小」</div>';
  html += '<div class="tip-content" style="white-space:pre-wrap">At AirAI, the scale was smaller — but the methodology is the same: identify the right use case, prove value through a pilot, drive adoption through training and feedback loops.\n\nMore importantly, at CINNOX I managed enterprise-level organizational transformation across 90 people and six cross-functional teams. The combination of hands-on AI deployment plus large-scale change management is what I bring to this role.</div>';
  html += '<div style="color:#6b7280;font-size:.8rem;margin-top:.5rem">AirAI 方法論 + CINNOX 企業級組織變革經驗</div>';
  html += '</div>';

  // Defense 3
  html += '<div class="tip-card" style="border-left-color:#ef4444">';
  html += '<div class="tip-title">「你不是 AI Architect」</div>';
  html += '<div class="tip-content" style="white-space:pre-wrap">You\'re right — and I believe that\'s actually the point of this role. The gap in most organizations isn\'t building AI. It\'s making AI work for people.\n\nMy value is in last-mile adoption — translating technology into real workflows, building trust with users, and making sure the change actually sticks. That\'s complementary to what the AI team does, not a duplication of it.</div>';
  html += '<div style="color:#6b7280;font-size:.8rem;margin-top:.5rem">價值在 last mile adoption，不是重複 AI team 的事</div>';
  html += '</div>';
